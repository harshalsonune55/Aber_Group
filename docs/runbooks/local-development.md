...
| Gap | Effect | Fix |
|---|---|---|
| No Docker | `make up`, `make odoo-up`, `make test-odoo` unavailable. Blocks Odoo work from M2. | Install Docker Desktop. |
...
**`make app-test-live` fails with "No backend".** The API is not running. Start
it with `make dev` in another terminal.

**The Android emulator cannot reach the API.** The emulator's `localhost` is the
emulator itself. `Env.apiBaseUrl` already falls back to `10.0.2.2` on Android;
